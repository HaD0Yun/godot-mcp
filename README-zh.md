...
> **设计哲学**：与其提供 90+ 个专用工具（如 `create_camera`、`create_light`、`create_physics_material`），GoPeak 使用通用的 `add_node` 和 `create_resource` 工具，适用于任意 Godot 类。AI 使用 `query_classes` 发现可用类型，使用 `query_class_info` 了解其属性 — 就像开发者使用 Godot 文档一样。
